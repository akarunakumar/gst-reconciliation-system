...
def test_upload_gstr2b_valid():
    with open(FIXTURES / "sample_gstr2b.xlsx", "rb") as f:
        resp = client.post("/api/v1/upload/gstr2b", files={"file": ("sample_gstr2b.xlsx", f, "application/octet-stream")})
    assert resp.status_code == 200
...
def _upload_both() -> tuple[str, str]:
    with open(FIXTURES / "sample_invoice.xlsx", "rb") as f:
        inv_resp = client.post("/api/v1/upload/invoices", files={"file": ("sample_invoice.xlsx", f, "application/octet-stream")})
    with open(FIXTURES / "sample_gstr2b.xlsx", "rb") as f:
        g2b_resp = client.post("/api/v1/upload/gstr2b", files={"file": ("sample_gstr2b.xlsx", f, "application/octet-stream")})
    return inv_resp.json()["temp_key"], g2b_resp.json()["temp_key"]


def test_start_reconciliation():
    inv_key, g2b_key = _upload_both()
    resp = client.post(
        "/api/v1/upload/start",
