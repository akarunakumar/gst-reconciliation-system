...
async def upload_invoices(file: UploadFile, _: dict = Depends(get_current_user)):
    return await _process_upload(file, "invoice")


@router.post("/gstr2b", response_model=UploadResponse)
async def upload_gstr2b(file: UploadFile, _: dict = Depends(get_current_user)):
    return await _process_upload(file, "gstr2b")


@router.post("/start", response_model=SessionResponse)
def start_reconciliation(body: StartReconciliationRequest, _: dict = Depends(get_current_user)):
